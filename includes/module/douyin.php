...
class Douyin {
	...
	public function find_video_id( $url ) {
		$url       = aiovd_unshorten( $url, $this->enable_proxies );
		$url       = strtok( $url, '?' );
	...
	public function media_info( $url ) {
		...
		$video["source_key"] = "douyin";
		$video["source_url"] = $url;
		$video["thumbnail"]  = $video_info["item_list"][0]["video"]["cover"]["url_list"][0];
		$video["duration"]   = aiovd_format_seconds( $video_info["item_list"][0]["video"]["duration"] / 1000 );
		...
		if ( ! empty( $music_url ) && ! empty( $video["links"][0] ) ) {
			...
			$video["links"][1]["size"]    = aiovd_get_file_size( $music_url, $this->enable_proxies );
